...
class ClipboardDB:
    ...
    def _init_db(self):
        ...
        # Create indices for recently_pasted
        cursor.execute(
            """
        ...
        # Create FTS5 virtual table for full-text search with name support
    ...
    def _cleanup_orphaned_pasted_records(self) -> int:
        """
        Clean up orphaned recently_pasted records that reference deleted clipboard items.
        This is a maintenance function to handle cases where foreign key constraints
        weren't properly enforced in older database versions.
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            DELETE FROM recently_pasted
            WHERE clipboard_item_id NOT IN (SELECT id FROM clipboard_items)
        """)
        ...
        if orphaned_count > 0:
            logging.info(f"Cleaned up {orphaned_count} orphaned recently_pasted records")
    ...
    def get_pasted_count(self) -> int:
        """Get total count of pasted items (excluding orphaned records)"""
        cursor = self.conn.cursor()
        # Only count pasted records that still have a valid clipboard item
        cursor.execute("""
            SELECT COUNT(*) as count FROM recently_pasted rp
